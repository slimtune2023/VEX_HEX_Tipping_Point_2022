...
motor LeftArmMotor = motor(PORT21, ratio36_1, true);
motor RightArmMotor = motor(PORT5, ratio36_1, false);
motor_group ArmMotors = motor_group(LeftArmMotor, RightArmMotor);
motor ClawMotor = motor(PORT16, ratio36_1, false);
inertial Inertial = inertial(PORT11);
motor BackClawMotor = motor(PORT10, ratio36_1, false);
...
int BCrotateDown() {
  BackClawMotor.rotateTo(-550, degrees);
...
int logarithmic(int speed) {
  ...
  int l_speed = sign * speed * speed / lim;
...
int rc_auto_loop_function_Controller1() {
  ...
  while(true) {
    if(RemoteControlCodeEnabled) {
      ...
      // left = Axis3 + Axis1
      // right = Axis3 - Axis1
      int temp1 = logarithmic(Controller1.Axis1.position());
      ...
      */
      ...
      if (Controller1.ButtonA.pressing()) {
        ...
        int lowerSpeed = temp3 * 25 / 127;

        if (temp3 > 15) {
          LeftDriveSmart.spin(forward, lowerSpeed, percent);
          RightDriveSmart.spin(forward, lowerSpeed, percent);

          isParked = false;
        } else if (temp3 < -15) {
          LeftDriveSmart.spin(reverse, -lowerSpeed, percent);
          RightDriveSmart.spin(reverse, -lowerSpeed, percent);
